...
export class Renderer {
  ...
  private textOrPlaceholder(text: string | undefined, placeholder: string) {
    if (text) {
  ...
  // [types]
  private typesList(types: string | undefined) {
    this.snippet.appendText(' [');
  ...
  private decorateSnippet() {
    ...
    lines = lines.filter((element, position, array) => {
      return (
        (position === 0 || element !== array[position - 1]) &&
        position < array.length - 1
      );
    });
    const decoratedLines: string =
      lines.map((line) => `#${line ? ` ${line}` : ''}`).join(this.eol) +
      this.eol;
